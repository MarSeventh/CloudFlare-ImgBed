...
export async function readIndex(context, options = {}) {
    try {
        ...
        if (publicAccess) {
            ...
            // 管理访问：安全优先
            if (!indexMeta.success) {
                shouldMerge = false;
            } else if (latestOperationId && indexMeta.lastOperationId) {
                shouldMerge = indexMeta.lastOperationId !== latestOperationId;
            } else if (!latestOperationId && !indexMeta.lastOperationId) {
                shouldMerge = false;
            } else {
        ...
        let index;
        if (publicAccess) {
            // 公开访问：索引缺失时返回空数据，不触发重建
            index = await loadChunkedIndex(context);
            if (!index.success) {
                console.log('Index not available for public access, returning empty result.');
                index = {
                    files: [],
